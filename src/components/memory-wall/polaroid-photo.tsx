...
export function PolaroidPhoto({ id, image, title, className, rotation = 0, delay = 0 }: PolaroidPhotoProps) {
  return (
    <Link
      href={`/posts/${id}`}
      className={cn(
        "group relative block w-[140px] sm:w-48 transition-all duration-900 hover:z-50 hover:scale-105",
    ...
    >
      {/* Wooden-style Clip */}
      <div className="absolute -top-3 sm:-top-4 left-1/2 -translate-x-1/2 w-2.5 sm:w-3 h-6 sm:h-8 bg-[#d4b08c] border border-black/10 shadow-sm z-20 rounded-sm overflow-hidden">
        <div className="absolute top-1/2 left-0 w-full h-[1px] bg-black/10" />
      </div>
 
      {/* Polaroid Frame */}
      <div className="bg-[#fdfdfd] p-2 sm:p-3 pb-8 sm:pb-10 shadow-[0_8px_16px_rgba(0,0,0,0.1)] border border-black/5 transform transition-all duration-700 group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.2)] group-hover:rotate-0">
        ...
        {/* Caption */}
        <p className="mt-2 sm:mt-3 text-center font-serif italic text-gray-600 text-[10px] sm:text-xs truncate px-1 max-w-[150px] mx-auto">
          {title}
        </p>
      </div>
